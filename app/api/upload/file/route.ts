...
async function uploadFile(req: NextRequest, user: any) {
  try {
    ...
    // Validate file size (max 50MB for guide files)
    ...
    const fileName = `${user.id}/guides/${Date.now()}-${Math.random().toString(36).substring(7)}.${fileExt}`;
...
export const POST = withAuth(uploadFile, ['admin', 'client']);
