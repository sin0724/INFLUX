-- ============================================
-- INFLUX 전체 마이그레이션 SQL (안전 버전)
-- DROP 구문 없이 안전하게 실행 가능
-- ============================================
...
-- RLS 정책 (안전 버전 - DROP 없음)
CREATE POLICY IF NOT EXISTS "Users can read own data" ON users
  FOR SELECT USING (true);

CREATE POLICY IF NOT EXISTS "Admins can manage users" ON users
  FOR ALL USING (true);

CREATE POLICY IF NOT EXISTS "Clients can read own orders" ON orders
  FOR SELECT USING (true);

CREATE POLICY IF NOT EXISTS "Admins can manage all orders" ON orders
  FOR ALL USING (true);
